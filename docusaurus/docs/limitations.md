...
5.  **Authentication and Access Control:** Phlox has built-in username/password accounts with admin and clinician roles, and **login is required for Docker deployments** (see [Users & Authentication](/authentication)). This is basic access control, **not** compliance-grade identity management:
    - **Docker/web:** first-run setup creates the admin account; further users are added by admins. There is no SSO/OIDC, MFA, or account-recovery flow.
    - **Desktop (Tauri):** a single implicit admin protected by a random local bearer token, plus a database passphrase.

    **Exposing Phlox to the public internet without a reverse proxy or VPN is highly discouraged and poses a significant security risk.** On desktop, the database passphrase must be **re-entered on every launch** — Phlox deliberately does not cache it in the OS keychain, so that physical access to the machine does not grant access to the database.
