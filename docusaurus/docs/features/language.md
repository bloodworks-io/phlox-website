...
Phlox can transcribe audio and generate clinical notes, letters, and chat responses in your working language. Each user picks their own **preferred language** in [Settings → User Settings → General](/settings#user-settings), and it drives transcription, LLM output, and date formatting for their account — clinicians in the same practice can each work in a different language.
...
- **Remote (Whisper-compatible)** — point Phlox at any endpoint in [Settings → Admin Settings → Whisper](/settings#remote-mode); remote endpoints generally support any language.
